...
class TestCsp(unittest.TestCase):
    def setUp(self):
        ...
        # a constraint that expects different variables to have
        # different values
        ...
        # a constraint thet expects two variables to be one odd and
        # the other even, no matter which one is which type
        def const_one_odd_one_even(variables, values):
            if values[0] % 2 == 0:
                # first even, expect second to be odd
                return values[1] % 2 == 1
            else:
                # first odd, expect second to be even
                return values[1] % 2 == 0
        ...
        self.problem = CspProblem(self.variables, self.domains,
                                  self.constraints)

    def test_most_constrained_variable_chooser(self):
        variable = _most_constrained_variable_chooser(self.problem,
                                                      self.variables,
                                                      self.domains)
        self.assertEquals(variable, 'C')

    def test_highest_degree_variable_chooser(self):
        variable = _highest_degree_variable_chooser(self.problem,
                                                    self.variables,
                                                    self.domains)
    ...
    def test_least_constraining_values_sorter(self):
        assignment = {'A': 1, 'B': 1}
        values = _least_constraining_values_sorter(self.problem, assignment,
                                                   'C', self.domains)
